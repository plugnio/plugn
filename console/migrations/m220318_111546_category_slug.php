...
/**
 * Class m220318_111546_category_slug
 */
class m220318_111546_category_slug extends Migration
{
    public $owner;
    ...
    public function safeUp()
    {
        ...
        {
            $batchQuery = $store->getCategories()
                ->andWhere(['like', 'slug', ""]);
            ...
            foreach ($batchQuery->batch(100) as $items)
            {
                foreach ($items as $item)
                {
                    ...
                    Yii::$app->db->createCommand('UPDATE category SET slug="'.$this->getSlugValue().'" 
                        WHERE category_id="'.$item->category_id.'"')->execute();
    ...
    {
        echo "m220318_111546_category_slug cannot be reverted.\n";
