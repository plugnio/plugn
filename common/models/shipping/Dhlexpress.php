...
class Dhlexpress
{
    public function getQuote($address) {
        ...
        $this->load->language('extension/shipping/dhlexpress');

        if($this->config->get('shipping_dhlexpress_realtime_rates') == true)
        {
            $status = true;
        }
        else
        ...
        if ($status) {
            ...
            $total_config_values = $this->get_currency();
            $mod_country = $this->config->get('shipping_dhlexpress_country_code');
            ...
            $dhl_packs		=	$this->get_dhl_packages( $get_product,$selected_currency );
            $total_value= 0;
            ...
            if (!$this->config->get('shipping_dhlexpress_test')) {
                $url = 'https://xmlpi-ea.dhl.com/XMLShippingServlet';
            } else {
                $url = 'https://xmlpitest-ea.dhl.com/XMLShippingServlet';
            }

            $pieces = $this->get_package_piece($dhl_packs);
            $weight_unit = ($this->config->get('shipping_dhlexpress_weight') == true) ? 'LB' : 'KG';
            $dim_unit = ($this->config->get('shipping_dhlexpress_weight') == true) ? 'IN' : 'CM';
            $fetch_accountrates = ($this->config->get('shipping_dhlexpress_rate_type') == 'ACCOUNT') ? "<PaymentAccountNumber>" . $this->config->get('shipping_dhlexpress_account') . "</PaymentAccountNumber>" : "";

            $mailing_date = date('Y-m-d');
            $mailing_datetime = date('c');
            $origin_postcode_city = $this->get_postcode_city($this->config->get('shipping_dhlexpress_country_code'), $this->config->get('shipping_dhlexpress_city'), $this->config->get('shipping_dhlexpress_postcode'));
            //$total_value = $this->cart->get_total();
            $is_dutiable = ($address['iso_code_2'] == $this->config->get('shipping_dhlexpress_country_code') || $this->dhl_is_eu_country($this->config->get('shipping_dhlexpress_country_code'), $address['iso_code_2'])) ? "N" : "Y";
            if($address['iso_code_2'] == 'AT' && $this->config->get('shipping_dhlexpress_country_code') == 'CZ'){
                $is_dutiable = "N";
            }
            if($address['iso_code_2'] == 'NL' && $this->config->get('shipping_dhlexpress_country_code') == 'SE'){
                $is_dutiable = "N";
            }
            ...
            $destination_postcode_city = $this->get_postcode_city($address['iso_code_2'], $to_city, $address['postcode']);
            $payment_country = $this->config->get('shipping_dhlexpress_country_code');
            ...
            $xml .= '					<SiteID>'.$this->config->get('shipping_dhlexpress_key').'</SiteID>';
            $xml .= '					<Password>'.$this->config->get('shipping_dhlexpress_password').'</Password>';
            $xml .= '			</ServiceHeader>';
            $xml .= '		</Request>';
            $xml .= '		<From>';
            $xml .= '			<CountryCode>'.$this->config->get('shipping_dhlexpress_country_code').'</CountryCode>';
    ...
    public function dhl_is_eu_country ($countrycode, $destinationcode) {
    ...
    public function get_dhl_packages($package,$orderCurrency,$chk = false)
    {
        switch ($this->config->get('shipping_dhlexpress_packing_type')) {
            case 'weight_based' :
    ...
    {
        $maximum_weight = ($this->config->get('shipping_dhlexpress_wight_b') !='') ? $this->config->get('shipping_dhlexpress_wight_b') : '50';
            ...
            foreach($packages as $package){//pre($package);
                ...
                $group = array(
                    ...
                    'Weight' => array(
                        ...
                        'Units' => ($this->config->get('shipping_dhlexpress_weight') == true) ? 'LBS' : 'KG'
                    ),
        ...
        foreach ($package as $item_id => $values) {
            ...
            if ($values['width'] && $values['height'] && $values['length']) {
                ...
                $group['Dimensions'] = array(
                    ...
                    'Units' => ($this->config->get('shipping_dhlexpress_weight') == true) ? 'IN' : 'CM'
                );
            }
            $group['packtype'] = $this->config->get('shipping_dhlexpress_per_item');
            $group['InsuredValue'] = $insurance_array;
    ...
    private function get_package_piece($dhl_packages) {
        $pieces = "";
        if ($dhl_packages) {
